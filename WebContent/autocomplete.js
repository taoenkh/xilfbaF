...
function handleLookup(query, doneCallback) {
	...
	if (query in mapstar){
		...
		console.log(mapstar);
		doneCallback( { suggestions: mapstar[query] } );
		console.log("cache2");
...
function handleLookupAjaxSuccess(data, query, doneCallback) {
	...
	console.log(query);
	console.log("In handlellooup");
	
	mapstar[query] = jsonData;
...
function handleSelectSuggestion(suggestion) {
	...
	console.log(url);
	if (suggestion["data"]["category"] === "Title"){
		
		window.location.replace("/cs122b/singletitle?title="+suggestion["value"]);
	}
	else if (suggestion["data"]["category"] === "Stars"){
		window.location.replace("/cs122b/singlestar?name="+suggestion["value"]);
...
function handleNormalSearch(query) {
	...
	window.location.replace("/cs122b/normalsearch?title="+query);
}
